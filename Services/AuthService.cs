...
public class AuthService : IAuthService
{
    ...
    public async Task<ApiResponse<string>> RegisterAsync(RegisterDto dto)
    {
        ...
        {
           return ApiResponse<string>.ErrorResponse($"Unexpected error in {nameof(AuthService)}.{nameof(RegisterAsync)}: {ex.Message}");
        }
    ...
    public async Task<ApiResponse<string>> LoginAsync(LoginDto dto)
    {
        ...
        {
            return ApiResponse<string>.ErrorResponse($"Unexpected error in {nameof(AuthService)}.{nameof(LoginAsync)}: {ex.Message}");
        }
    }
    ...
    private string GenerateJwtToken(User user)
    {
        ...
        {
            throw new Exception($"Unexpected error in {nameof(AuthService)}.{nameof(GenerateJwtToken)}: {ex.Message}");
        }
    }
    public async Task<ApiResponse<Dictionary<string, string>>> GetUserNamesAsync()
    {
        ...
        {
            return ApiResponse<Dictionary<string, string>>.ErrorResponse($"Unexpected error in {nameof(AuthService)}.{nameof(GetUserNamesAsync)}: {ex.Message}");
        }
    }
}